import React, { useState } from 'react';
import { Link } from 'react-router-dom';
import { Leaf, Mail, ArrowRight, ArrowLeft, CheckCircle2 } from 'lucide-react';

export default function ForgotPassword() {
  const [isSubmitted, setIsSubmitted] = useState(false);

  return (
    <div className="min-h-[80vh] flex items-center justify-center p-4">
      <div className="bg-white w-full max-w-md rounded-3xl shadow-xl border border-slate-100 overflow-hidden">
        <div className="p-10">
          <div className="flex flex-col items-center mb-10">
            <div className="bg-slate-grey p-3 rounded-2xl mb-4">
              <Leaf className="text-energetic-teal w-8 h-8" />
            </div>
            <h1 className="text-2xl font-black text-slate-grey uppercase tracking-tight text-center">
              {isSubmitted ? 'Check Your Email' : 'Reset Password'}
            </h1>
            <p className="text-gray-500 mt-2 text-sm font-medium text-center">
              {isSubmitted 
                ? "We've sent recovery instructions to your email address." 
                : "Enter your email and we'll send you instructions to reset your password."}
            </p>
          </div>

          {!isSubmitted ? (
            <form className="space-y-6" onSubmit={(e) => { e.preventDefault(); setIsSubmitted(true); }}>
              <div className="space-y-1.5">
                <label className="text-xs font-bold text-gray-400 uppercase tracking-widest px-1">Email Address</label>
                <div className="relative group">
                  <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                    <Mail className="h-5 w-5 text-gray-400 group-focus-within:text-energetic-teal transition-colors" />
                  </div>
                  <input 
                    type="email" 
                    required
                    className="block w-full pl-11 pr-4 py-3.5 bg-slate-50 border border-slate-100 rounded-xl text-slate-grey placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-energetic-teal/20 focus:border-energetic-teal transition-all" 
                    placeholder="name@farm.com"
                  />
                </div>
              </div>

              <button className="w-full bg-slate-grey text-white py-4 rounded-xl font-bold flex items-center justify-center gap-2 hover:bg-slate-800 transition-all shadow-lg active:scale-[0.98]">
                <span>SEND RESET LINK</span>
                <ArrowRight size={18} />
              </button>
            </form>
          ) : (
            <div className="bg-energetic-teal/5 border border-energetic-teal/20 rounded-2xl p-6 flex flex-col items-center text-center">
              <CheckCircle2 className="text-energetic-teal w-12 h-12 mb-4" />
              <p className="text-slate-grey font-bold mb-1">Email Sent!</p>
              <p className="text-sm text-gray-500 font-medium">Please check your inbox (and spam folder) for the reset link.</p>
            </div>
          )}

          <div className="mt-10 pt-8 border-t border-slate-50 text-center">
            <Link 
              to="/signin" 
              className="inline-flex items-center gap-2 text-sm text-gray-500 font-bold hover:text-slate-grey transition-colors"
            >
              <ArrowLeft size={16} />
              <span>Back to Sign In</span>
            </Link>
          </div>
        </div>
      </div>
    </div>
  );
}
